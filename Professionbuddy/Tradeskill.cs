...
namespace HighVoltz
{
    ...
    public class TradeSkill
    {
        ...
        static TradeSkill()
        {
                ...
                GlobalPBSettings.Instance.KnownSpellsPtr == 0)
            {
                ...
                try
                {
                    uint pointer = Util.FindPattern("94 1E B3 00 8B CB 83 E1 1F B8 01 00 00 00 D3 E0 8B CB C1 E9 05 85 04 8A 0F 95 C0 84 C0 75",
                        "????xxxxxxxxxxxxxxxxxxxxxxxxxx");
                    GlobalPBSettings.Instance.KnownSpellsPtr = ObjectManager.Wow.Read<uint>(baseAddress + pointer) - baseAddress;
                ...
                {
                    Professionbuddy.Log("Unable to find KnownSpellsPtr offset for WoW Version {0}\nPlease notify the developer of this issue",
                        mod.FileVersionInfo.FileVersion);
                }
            }
        }

        TradeSkill(WoWSkill skill)
        {
            WoWSkill = skill;
        ...
        List<SkillLineAbilityEntry> GetSkillLineAbilityEntries()
        {
            ...
            var firstRowPtr = ObjectManager.Wow.Read<uint>(((uint)ClientDb.SkillLineAbility + ObjectManager.Wow.ImageBase) + 0x14);
            uint id;
            // optimized search
            do
            {
            ...
            uint index = ObjectManager.Wow.Read<uint>((firstRowPtr + (half - 1) * SkillLineAbilityEntrySize));
        ...
        /// <returns></returns>
        static public TradeSkill GetTradeSkill(SkillLine skillLine)
        {
            if (!ObjectManager.IsInGame)
        ...
        /// <returns></returns>
        static public bool HasSpell(uint spellId)
        {
    ...
    public class Recipe
    {
                ...
                if (spelldbRow != null)
                {
                        ...
                        reagentIndex >= reagentDbTable.MinIndex)
                    {
                        ...
                        for (uint index = 1; index <= 8; index++)
                        {
                            ...
                            if (id != 0)
                            {
                                _ingredients.Add(new Ingredient(id, reagentDbRow.GetField<uint>(index + 8), _parent.Ingredients));
                            }
                        }
                    }
                }
        ...
        public void UpdateHeader()
        {
            ...
            if (CraftedItemID == 0)
            {
                dbTable = StyxWoW.Db[Styx.Patchables.ClientDb.SkillLine];
            ...
            else
            {
                ...
                var cache = StyxWoW.Cache[Styx.WoWInternals.WoWCache.CacheDb.ItemClass].GetInfoBlockById(CraftedItemID);

                if (cache != null)
                {
                    dbTable = StyxWoW.Db[Styx.Patchables.ClientDb.ItemSubClass];
        ...
        Tool GetTool(uint index, Tool.ToolType toolType)
        {
            ...
            var tool = _parent.Tools.Find(a => a.Equals(newTool));
            if (tool == null)
            {
                tool = newTool;
    ...
    struct SkillLineAbilityEntry
    {
        ...
        {
            return string.Format("{0} SkillLine: {1},NextSpellId: {2},AquireMethod: {3},OrangeSkillLevel: {4},YellowSkillLevel: {5}\n\tGreenSkillLevel: {6},GreySkillLevel: {7},SkillPointsEarned: {8},DisplayOrder: {9}",
